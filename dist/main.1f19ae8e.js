...
var api = jQuery(".test"); //对象里的addClass方法可以给所有获取到的元素增加类，这里就是给所有类为test的元素再加一个类：red

api.addClass("red"); //上边添加类red后返回的东西还是一个对象（这个对象里有很多函数/方法），所以可以继续链式操作：
...
jQuery(".test").find(".child").addClass("red").addClass("blue").addClass("green").end() //end方法表示结束当前的api对象，返回上一层api对象进行操作
.addClass("yellow"); //此时的addClass方法操作的是上一层，即把yellow类加到.test元素身上;
//找到所有的类为chlid的元素，并调用each方法遍历这些元素

var x = jQuery(".test").find(".child"); //each方法又接受一个函数fn，fn会打印出这些类为.child的元素
