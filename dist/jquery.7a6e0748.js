...
window.jQuery = function (selectorOrArray) {
  var elements; //变量作用域提升，因为在if里声明的话就只能作用在if内部了

  if (typeof selectorOrArray === "string") {
    //elements是一个数组，包含获取到的所有selector元素
    elements = document.querySelectorAll(selectorOrArray); //jQuery获取元素selector,但是并不能返回这个元素，只能返回一个对象，通过对象里的函数（方法）可以操控这个元素
  ...
  return {
    ...
    find: function find(selector) {
      var array = []; //遍历elements数组

      for (var i = 0; i < elements.length; i++) {
        //在elements数组的每个元素里找到selector对应的所有元素,这些元素是一个伪数组；
        // 用Array.from变成真正的数组，并赋值给elements2
        var elements2 = Array.from(elements[i].querySelectorAll(selector)); //再把空数组连接伪数组，合起来组成新的数组，这个新数组里就是找到的所有selector元素
